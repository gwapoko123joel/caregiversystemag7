      ...
      <div className="flex items-center justify-between px-2">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-[10px] font-black text-sky-500 uppercase tracking-[0.4em]">Protocol: Clinical Transfer</span>
          </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-slate-900/40 backdrop-blur-md border border-white/5 rounded-[40px] p-8 shadow-2xl">
          <div className="flex items-center gap-3 mb-8">
             ...
             <h3 className="text-xs font-black text-white uppercase tracking-[0.2em]">Subject Identification</h3>
          </div>
        ...
        <div className="bg-slate-900/40 border border-white/5 rounded-[40px] p-8 shadow-sm">
          <div className="flex items-center gap-3 mb-8">
             ...
             <h3 className="text-xs font-black text-white uppercase tracking-[0.2em]">Clinical Snapshot</h3>
          </div>
        ...
        <div className="bg-slate-900/40 backdrop-blur-md border border-white/5 rounded-[40px] p-8">
          <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-6">Urgency Threshold</h3>
        ...
        <div className="bg-slate-900/60 backdrop-blur-md border border-white/5 rounded-[40px] p-8">
          <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-6">Clinical Justification</h3>
          <textarea 
            required
            placeholder="Document reason for transfer, initial diagnosis, or critical context..."
            value={formData.clinicalNotes}
            onChange={(e) => setFormData({...formData, clinicalNotes: e.target.value})}
            className="w-full bg-slate-950/50 border border-white/10 rounded-[32px] p-6 text-sm text-white focus:border-sky-500/50 transition-all outline-none min-h-[160px] resize-none placeholder:text-slate-700"
          />
        </div>

        {error && (
          <div className="p-4 bg-rose-500/10 border border-rose-500/20 rounded-2xl text-rose-500 text-[10px] font-black uppercase tracking-widest">
            {error}
          </div>
        )}
        ...
        <div className="flex flex-col md:flex-row items-center gap-4 pt-4">
           <button 
             type="button" 
             onClick={onBack}
             className="w-full md:w-1/4 py-5 bg-white/5 border border-white/10 text-slate-400 rounded-3xl text-[10px] font-black uppercase tracking-widest hover:text-white transition-all"
           >
             Cancel
           </button>
           <button 
             type="submit"
             disabled={loading}
             className="w-full md:w-3/4 py-5 bg-sky-500 hover:bg-sky-400 text-white rounded-3xl text-[10px] font-black uppercase tracking-[0.3em] transition-all shadow-xl shadow-sky-500/30 active:scale-[0.98] flex items-center justify-center gap-3"
           >
    ...
    <div className="space-y-2">
      ...
      <div className="relative group">
        ...
        <input 
          ...
          className={`w-full bg-slate-950/50 border border-white/10 rounded-2xl py-4 ${icon ? 'pl-12' : 'px-5'} pr-5 text-sm text-white outline-none focus:border-sky-500/50 transition-all placeholder:text-slate-800 font-medium ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}
        />
      </div>
    </div>
  );
...
function VitalsInput({ label, unit, value, onChange, color = "text-white" }: any) {
  return (
    <div className="bg-slate-950/50 border border-white/5 p-4 rounded-3xl text-center flex flex-col items-center justify-center group hover:border-sky-500/30 transition-all">
       <p className="text-[8px] font-black text-slate-600 uppercase mb-1">{label} ({unit})</p>
       <input 
         value={value}
         onChange={(e) => onChange?.(e.target.value)}
         className={`bg-transparent text-center text-lg font-mono font-black outline-none w-full ${color}`}
       />
    </div>
  );
}
...
function UrgencyBtn({ label, active, onClick, color }: any) {
  ...
  const activeThemes: any = {
    emerald: 'border-emerald-500 bg-emerald-500 text-white shadow-lg shadow-emerald-500/20',
    amber: 'border-amber-500 bg-amber-500 text-white shadow-lg shadow-amber-500/20',
    rose: 'border-rose-500 bg-rose-500 text-white shadow-lg shadow-rose-500/20',
  };
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`py-4 rounded-2xl border-2 font-black uppercase text-[10px] tracking-widest transition-all ${active ? activeThemes[color] : themes[color] + ' opacity-40 hover:opacity-100'}`}
    >
      {label}
    </button>
  );
