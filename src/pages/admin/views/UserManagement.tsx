      ...
      <div className="bg-card border border-card-border rounded-[40px] p-8 lg:p-12 relative overflow-hidden shadow-sm dark:shadow-none transition-colors">
        <div className="absolute top-0 right-0 p-8">
           <button 
             ...
             className="px-6 py-3 bg-sky-500 text-white font-black uppercase tracking-widest text-xs rounded-2xl shadow-harmonized flex items-center gap-2 hover:scale-105 active:scale-95 transition-all"
           >
             <Plus size={16} /> Authorize Personnel
           </button>
        </div>
        
        <h2 className="text-3xl font-black text-text-main mb-2 tracking-tight uppercase transition-colors">Access Governance</h2>
        <p className="text-sidebar-text-muted font-bold text-sm uppercase tracking-widest mb-12 transition-colors">User Provisioning & Authorization Matrix</p>

        {/* Enrollment Form */}
        {showAddUser && (
          <div className="mb-12 p-8 bg-card border border-card-border rounded-3xl animate-in zoom-in-95 duration-300 shadow-sm dark:shadow-none">
            <h4 className="text-xs font-black uppercase text-sky-500 tracking-[0.2em] mb-6">Initialize New Node Authorization</h4>
            ...
            <div className="flex gap-4 mt-8">
               <button 
                 ...
                 className="px-8 py-3 bg-sky-500 text-white font-black text-xs uppercase tracking-widest rounded-xl hover:shadow-[0_0_20px_rgba(0,229,255,0.4)] hover:scale-105 active:scale-95 transition-all"
               >
                 Initialize Credentials
               </button>
               <button onClick={()=>setShowAddUser(false)} className="px-8 py-3 text-xs font-black text-sidebar-text-muted hover:text-text-main uppercase tracking-widest transition-colors">Abort</button>
            </div>
          </div>
        )}
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
           <div className="relative group max-w-sm flex-1">
              ...
              <input 
                ...
                className="w-full bg-card border border-card-border rounded-2xl py-3 pl-12 pr-4 text-xs font-semibold text-text-main focus:outline-none focus:border-sky-500/40 placeholder:text-sidebar-text-muted/50 tracking-wider transition-colors shadow-sm dark:shadow-none" 
              />
           </div>
        </div>
