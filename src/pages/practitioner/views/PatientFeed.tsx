      ...
      <div className="bg-card border border-card-border rounded-[40px] p-8 lg:p-12 shadow-sm dark:shadow-none transition-colors">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8 mb-12 transition-colors">
           <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-xl font-black text-text-main uppercase tracking-tight leading-none italic transition-colors">Patient Roster</h3>
           ...
           <div className="relative group max-w-sm flex-1">
               ...
               <input 
                 ...
                 className="w-full bg-card border border-card-border rounded-2xl py-4 pl-12 pr-4 text-xs font-semibold text-text-main focus:outline-none focus:border-sky-500/40 placeholder:text-sidebar-text-muted/50 tracking-wider transition-colors shadow-sm dark:shadow-none" 
               />
           </div>
        </div>
                    ...
                    >
                       ...
                       <div className="p-6 flex-1 flex flex-col">
                          <div className="flex justify-between items-start mb-4">
                             <div>
                                <h3 className="text-lg font-black text-text-main italic tracking-tight leading-none transition-colors">{p.first_name} {p.last_name}</h3>
                                <div className="text-[9px] font-bold text-sidebar-text-muted uppercase tracking-widest mt-2 group-hover:text-sky-500 dark:group-hover:text-sky-400 transition-colors">PT-{p.patient_id.toString().padStart(4, '0')}</div>
                             </div>
                             <div className="w-8 h-8 rounded-full border border-card-border flex items-center justify-center text-sidebar-text-muted group-hover:bg-sky-500 group-hover:text-white group-hover:border-transparent transition-all shadow-xl shadow-sky-500/10">
                                <ArrowUpRight size={14} />
                             </div>
                          </div>
                          <div className="mt-auto space-y-2">
                             <div className="flex justify-between text-[10px] font-black uppercase tracking-widest border-t border-card-border pt-4 transition-colors">
                                <span className="text-sidebar-text-muted transition-colors">Last Check-in</span>
