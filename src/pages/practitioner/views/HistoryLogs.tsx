      ...
      <div className="bg-card border border-card-border rounded-[32px] md:rounded-[40px] p-6 lg:p-12 shadow-sm dark:shadow-none transition-colors">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 md:gap-8 mb-8 md:mb-12 transition-colors">
           <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg md:text-xl font-black text-text-main uppercase tracking-tight leading-none italic transition-colors">Historical Audit Trail</h3>
           ...
           <div className="relative group w-full md:max-w-sm">
               ...
               <input placeholder="SEARCH CLINICAL LOGS..." className="w-full bg-card border border-card-border rounded-2xl py-4 pl-12 pr-4 text-xs font-semibold text-text-main focus:outline-none focus:border-sky-500/40 placeholder:text-sidebar-text-muted/50 tracking-wider transition-colors shadow-sm dark:shadow-none" />
           </div>
        </div>
                 ...
                 {logs.length === 0 ? (
                    <tr>
                      <td colSpan={4} className="px-8 py-20 text-center">
                        <div className="text-[10px] font-black text-sidebar-text-muted uppercase tracking-widest italic transition-colors">Node telemetry archive is currently empty.</div>
                      </td>
                    </tr>
                 ) : (
                    logs.map(log => (
                       <tr key={log.log_id} className="group hover:bg-card/50 transition-all">
                          <td className="px-8 py-8">
                             <div className="flex items-center gap-4">
                                ...
                                <div>
                                   <div className="text-sm font-black text-text-main italic tracking-tight transition-colors">{log.patient_name || '—'}</div>
                             ...
                             <div className="flex flex-wrap gap-4">
                                {[
                                   ...
                                   <div key={v.label} className="bg-card border border-card-border px-3 py-1.5 rounded-xl flex items-center gap-2 transition-colors">
                                      ...
                                      <span className="text-[10px] font-black text-text-main font-mono leading-none tracking-tighter transition-colors">{v.val}</span>
                                   </div>
                                ))}
                             </div>
                          </td>
                          <td className="px-8 py-8">
                             <div className="text-[10px] font-black text-sidebar-text-muted uppercase tracking-widest transition-colors">{log.caregiver_name || 'System Admin'}</div>
                          </td>
                          <td className="px-8 py-8 text-right">
                             <div className="text-sm font-black text-text-main font-mono tracking-widest leading-none transition-colors">
        ...
        <div className="md:hidden space-y-4">
           {logs.length === 0 ? (
              <div className="p-12 text-center bg-card border border-card-border rounded-3xl">
                 <div className="text-[10px] font-black text-sidebar-text-muted uppercase tracking-widest italic">Node telemetry archive empty.</div>
              </div>
           ) : (
              logs.map(log => (
                 <div key={log.log_id} className="bg-card border border-card-border rounded-[32px] p-6 shadow-sm active:scale-[0.98] transition-all">
                    <div className="flex items-start justify-between mb-6">
                       <div className="flex items-center gap-3">
                          ...
                          <div>
                             <div className="text-base font-black text-text-main italic tracking-tight">{log.patient_name || '—'}</div>
                       ...
                       <span className={`px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-tight ${log.physical_status === 'critical' ? 'bg-alert-text/10 text-alert-text' : 'bg-sky-500/10 text-sky-500'}`}>
                          {log.physical_status}
                       </span>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-2 mb-6">
                       {[
                          ...
                          <div key={v.label} className="bg-primary/30 border border-card-border p-2 rounded-2xl flex flex-col items-center justify-center gap-1">
                             ...
                             <span className="text-[10px] font-black text-text-main font-mono">{v.val || '—'}</span>
                          </div>
                       ))}
                    </div>

                    <div className="flex items-center justify-between pt-4 border-t border-card-border">
                       <div className="text-[9px] font-black text-sidebar-text-muted uppercase tracking-widest">Caregiver</div>
                       <div className="text-[10px] font-black text-text-main uppercase">{log.caregiver_name || 'System Admin'}</div>
                    </div>
                 </div>
              ))
           )}
